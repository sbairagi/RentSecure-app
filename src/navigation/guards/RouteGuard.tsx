...
import { matchRoute, isAuthRoute, isPublicRoute, checkRoutePermission } from '@/navigation/utils/routeUtils';
import { isSubscriptionExpired } from '@/navigation/utils/subscription';
import { apiService } from '@/services/api/apiClient';
import { useAuthStore } from '@/store/authStore';
import { useSubscriptionStore } from '@/store/subscriptionStore';
import { navigationReadyStore } from '@/navigation/navigation-state';
...
export function RouteGuard({
  ...
  const canAccessRoute = useCallback(
    (route: string): boolean => {
      ...
      const isAuthRoutePath = isAuthRoute(route) || route === SPLASH_ROUTE;
      ...
      if (requireFeature) {
        const features = ROLE_FEATURE_ACCESS[currentRole] || [];
        if (!features.includes('*') && !features.includes(requireFeature)) {
          return false;
        }
      }
      ...
      return checkRoutePermission(matchedRoute, currentRole);
    },
    [
      isAuthenticated,
      currentRole,
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      const isPublicRoutePath = isPublicRoute(pathname) || isAuthRoute(pathname);
