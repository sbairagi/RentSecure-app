...
export default function UpgradePlanScreen() {
  ...
  const availablePlans = plans?.filter(p => subscriptionService.canUpgrade(currentPlanName, p.name)) ?? [];
          ...
          {availablePlans.length > 0 ? (
            availablePlans.map(plan => (
              <PlanCard
                ...
                onUpgrade={() => router.push('/(drawer)/(tabs)/subscription/purchase-confirmation' as any)}
              />
            ))
          ) : (
            <EmptyState
